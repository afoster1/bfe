...
bfe.toolbox.rsync.transfer()
{
    ...
    bfe.toolbox.utils.run "${MKDIR_CMD} -p ${dest_dir}"
    ...
    # the problems.
    bfe.toolbox.utils.run "${RSYNC_CMD} --checksum --ignore-times --itemize-changes -rlptD --delete-after --copy-links --times --verbose ${filters} ${source_dir} ${dest_dir}"
}
